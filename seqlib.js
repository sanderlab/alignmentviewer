...
function initGraph(msa){
	var svg = d3.select("#MSApairwiseGraph2"),
	...
	for (var elm in msa.pwhash){
		if (i < msa.h) {
			var singleObj = {};
		    singleObj['id'] = i;
			graph0.nodes.push(singleObj);
		}
		i = i + 1
		sp = key.split(',');
		var val = msa.pwhash[key];
		...
		if (val > 0.3){
			...
			sNode['id'] = sp[0];
			tNode['id'] = sp[1];
		    parentNode['source'] = sNode;
		    parentNode['target'] = tNode;
		    parentNode['value'] = 1 - val;
			graph0.links.push(parentNode);
		}
	}

	var graph = JSON.stringify(graph0);
	...
	function generateGraph(graph) {
	  ...
	  var node = svg.append("g")
	    ...
	    .enter().append("circle")
	      .attr("r", 5)
	      .attr("fill", function(d) { return "rgb(0, 69, 41)"; })
	      .call(d3.drag()
